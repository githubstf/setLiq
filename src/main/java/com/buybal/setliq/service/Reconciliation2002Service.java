...
import com.buybal.setliq.receivemail.service.impl.ReceiveMail2002ServiceImpl;
import com.buybal.setliq.receivemail.service.impl.ReconciliationCheck2002ServiceImpl;
import com.buybal.util.PropertiseUtil;

public class Reconciliation2002Service {
	Logger logger = LoggerFactory.getLogger(Reconciliation2002Service.class);
	private String bank = PropertiseUtil.getString("setLiq", "2002");
	private TbankchefileService bcf = new TbankchefileService();
	private ReconciliationCheckService rCkService = new ReconciliationCheck2002ServiceImpl();
	//下载邮件
	public void receiveMail(){
		ReceiveMailService service = new ReceiveMail2002ServiceImpl();
   ...
   //加载
	...
	private int reconciliationCheck(Tbankchkfile file) {
		...
		String liqDate= fileName.substring(5,5+8);
