 ...
 * @Description: 机构对数据进行解析验签和入库
 * @author Hippo
 * @date 2014-9-19
 * @version 1.0
 */
public class ParseOrgFileService {
	private static final Logger logger = Logger.getLogger(ParseOrgFileService.class);
	 ...
	 */
	public boolean doParseFile(String localFileName, String liqDate,String bankId) {
		...
		if (sl.length != 3) {
			logger.error("[文件解析Service]文件名格式错误");
			return false;
		}
		String orgCode = sl[0];
		...
		try {
			...
			while ((lineData = bufferReader.readLine()) != null) {
				if (!verifySign(orgCode, lineData)) {
			...
			bufferReader.reset();
			while ((lineData = bufferReader.readLine()) != null) {
				if (!insertToDB(lineData,bankId,orgCode)) {
	 ...
	 * @return
	 */
	private boolean verifySign(String orgCode, String lineData) {
		purCnt++;
	...
	/**
	 * 解析文件入库
	 * @param lineData 文件中一行的数据
	 * @param bankId 银行编号
	 * @param orgCode 机构编号
	 * @return
	 */
	private boolean insertToDB(String lineData,String bankId,String orgCode) {
		...
		try{
			...
			logger.error("入库失败lineData="+lineData,e);
		}
		return true;
	}
