...
import com.buybal.setliq.runnable.MerReconLiqRunnable;
 ...
 * @author hippo
 */
@WebServlet("/MerReconServlet")
public class MerReconServlet extends HttpServlet {
	private static final Logger logger = LoggerFactory.getLogger(MerReconServlet.class);
	private static final long serialVersionUID = 1L;
       
    public MerReconServlet() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			String localFileName = Tdatadic.getDIC_NAME() + "bankRecon/" + merId + "/"+fileName; // 文件本地保存地址
			...
			// 启动处理线程
			Runnable run = new MerReconLiqRunnable(merId, remoteFileName, localFileName, liqDate,bankId);
			run.run();
		} catch (Exception e) {
