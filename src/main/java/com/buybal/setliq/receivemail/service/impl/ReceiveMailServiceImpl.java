...
public class ReceiveMailServiceImpl implements ReceiveMailService {
	...
	private static Logger logger = LoggerFactory.getLogger(ReceiveMailServiceImpl.class);
	 ...
	 * @return 无
	 */
	public void receiveMail(String userName, String passWord, String mailHost, Integer port, String bankId) {
	 ...
	 * @return
	 */
	public String filterHtml(String regxpForHtml, String str) {
	...
	public String getMailContent(Part part) {
		try {
		...
		if (part.isMimeType("multipart/*")) {
			...
			for (int i = 0; i < mp.getCount(); i++) {
				...
				if ((disposition != null) && ((disposition.equals(Part.ATTACHMENT)) || (disposition.equals(Part.INLINE)))) {
					fileName = mpart.getFileName();
					if (fileName != null) {
						if (fileName.contains(".") && fileName.contains("_") && fileName.substring(0, fileName.indexOf(".")).split("_")[0].length() == 15
								&& fileName.substring(0, fileName.indexOf(".")).split("_")[1].length() == 8) {
	...
	private int saveMail(Map<String, Object> map) throws UnsupportedEncodingException {
		...
		for (int i = 0; i < files.size(); i++) {
			...
			fileName = MimeUtility.decodeText(fileName);
			/* String namestr = fileName.substring(fileName.lastIndexOf("_")+1,fileName.lastIndexOf(".")); */
			/*
			 * namestr = DateUtil.fomatDate(DateUtil.getAnyDayByNo( DateUtil.StringToDate(namestr, "yyyyMMdd"), -1), "yyyyMMdd");
			 */
